...
def carregar_lista_dinamica_smart(textos):
    ...
    with st.spinner(f"{textos['status_minerando']} {alvo}..."):
        ...
        if novos: lista_mestra.extend(novos)
            ...
            for lista in c.PRESETS_FRONTEIRA.values(): lista_mestra.extend(lista)
...
if st.session_state.pagina == 'resultados':
    ...
    if df is not None and not df.empty:
        top = df.iloc[0]; c1, c2, c3 = st.columns(3)
        c1.metric(t["metric_top"], top[t["col_mol"]], delta=top[t["col_status"]])
        c2.metric(t["metric_score"], top[t["col_ratio"]]); c3.metric("P-Valor", top["P-Value"])
        ...
        st.divider(); st.subheader(t["header_leitura"])
    ...
    with col_main:
        ...
        with st.expander(t["expander_presets"], expanded=False):
            if st.button(t["btn_add_all"], use_container_width=True):
                todos = []; [todos.extend(l) for l in c.PRESETS_FRONTEIRA.values()]
                adicionar_termos_seguro(todos, t); st.rerun()
            st.divider(); escolha = st.selectbox(t["label_categoria"], options=list(c.PRESETS_FRONTEIRA.keys()))
