...
# --- 1. MOTOR DE MÉTRICAS (RIGOR CIENTÍFICO) ---
...
# --- 2. HIGIENIZAÇÃO (EVITA ERRO 400 NO PUBMED) ---
...
def buscar_todas_noticias(lang='pt'):
    try:
        # Busca focada em novidades de 2025 para a bexiga/farmaco
        query = "(bladder physiology OR pharmacology OR molecular biology) AND (2024:2025[Date - Publication])"
...
# --- 4. DOUTORA INVESTIGADORA (COM CRUZAMENTO RÍGIDO) ---
def _doutora_investigadora(termo_base, lista_pubmed=None, fase="brainstorming"):
    ...
    if fase == "brainstorming":
        prompt = f"Como PhD em farmacologia, liste 50 alvos moleculares (canais, receptores, fármacos específicos) inovadores para {termo_base}. Ex: TRPV4, GSK1016790A, SPHK1. Apenas lista Python."
    else:
        lista_str = ", ".join(lista_pubmed)
        prompt = f"""
        Tu é uma Investigadora Sênior. Cruze seu conhecimento com estes termos do PubMed: {lista_str}.
        MISSÃO: Isolar apenas alvos de bancada e fármacos.
        REGRAS DE OURO:
        - Delete siglas de 1 ou 2 letras (J, S, II, IV, O, T).
        - Delete termos genéricos (Study, Results, MRNA, DNA, Treatment, Body, Comparison).
        - Delete termos de fisiologia antiga (Toad, Rabbit, Skin).
        - Priorize fármacos (ex: Mirabegron, GSK1016790A, Solifenacin).
...
def buscar_alvos_emergentes_pubmed(termo_base, email, usar_ia=True):
    ...
    try:
        ...
        # Filtro de Regex: Apenas palavras com 3+ letras
